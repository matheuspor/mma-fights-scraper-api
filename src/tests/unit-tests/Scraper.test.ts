...
describe('Tests Scraper class', () => {
  it('Tests Scraper constructor', () => {
    ...
    expect(scraper.UFCData.events).to.be.deep.equal([])
    expect(scraper.UFCData.eventCard).to.be.deep.equal([])
  })
  describe('Tests setup method', () => {
    ...
    it('Tests setup method when currentDate is greater than deployedDate', async () => {
      ...
      expect(scraper.UFCData.events).to.be.equal('scrapedEvents')
      expect(scraper.UFCData.eventCard).to.be.equal('scrapedEventsFights')
    })
